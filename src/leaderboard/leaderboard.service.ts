...
export class LeaderboardService {
  private scores: Map<string, Map<string, { username: string; score: number }>> = new Map();

  async addScore(contestId: string, userId: string, username: string, points: number): Promise<number> {
    if (!this.scores.has(contestId)) {
      this.scores.set(contestId, new Map());
    }
    const contest = this.scores.get(contestId)!;
    const current = contest.get(userId)?.score ?? 0;
    const newScore = current + points;
    contest.set(userId, { username, score: newScore });
    return newScore;
  }

  async getTopN(contestId: string, topN = 10): Promise<LeaderboardEntry[]> {
    const contest = this.scores.get(contestId);
    if (!contest) return [];

    return Array.from(contest.entries())
  ...
  async getAll(contestId: string): Promise<LeaderboardEntry[]> {
    return this.getTopN(contestId, 9999);
  }

  async clearContest(contestId: string): Promise<void> {
    this.scores.delete(contestId);
  }
}